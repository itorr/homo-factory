const homo = ((numsTable) => {
	...
	const breakNumber = (num) => {
		...
		if (!Number.isInteger(num)) {
			...
			return `(${breakNumber(num * Math.pow(10, n))})/(10)^(${n})`
		}
		...
		// 将不在表内的数字n转换成 a*[n/a]+b 的形式, 其中[x]为最大不超过x的整数, a为表内数字
        // 递归处理 [n/a] 和 b 
		...
		return rst.replaceAll("*(1)", "").replaceAll("+(0)", "").replaceAll("^(1)", "")
	}

	const finisher = (expr) => {
		// 将所有表内值转换为具体记录的表达式并且把^换成**
		expr = expr.replace(/\d+|⑨/g, (n) => numsTable[n]).replaceAll("^", "**")
